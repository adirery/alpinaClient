...
object RequestResponseClient {
  ...
  def send(protocol:String, host:String, metrics:Seq[AvroApiMetrics])(implicit ec:ExecutionContext,
    ...
    val f = for {
      response <- sttp
        .post(uri"$protocol://$host/custodian-event/metrics")
   ...
   f.onComplete{
      case Success(r) =>println(s" success resp from metrics endpoint $r")
      case Failure(t) => println(s" failed resp from metrics endpoint $t")
